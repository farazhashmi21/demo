/* global corporateIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'corporate-supports-dark-theme' ) ) {
	corporateDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function corporateDarkModeEditorInit( attempt ) {
	...
	if ( corporateIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						corporateDarkModeEditorInit( attempt + 1 );
					},
