                ...
                if($engines[0] == 'gpt'){
                    if(empty($_COOKIE["last_five_prompt"])){
                        array_push($gptkeyword, array(
                         ...
                         setcookie('last_five_prompt', (json_encode($gptkeyword)) , time() + (60000), "/");
                    }else{
                       $json_string = ($_COOKIE['last_five_prompt']);
                       // setcookie('last_five_prompt', null , time() + (60000), "/");
                        $data = stripcslashes($json_string);
                      
                       $gptkeyword = json_decode(($data));
                    ...
                    if(get_option('conversation_continuity') == 1){
                        ...
                        if(is_array($gptkeyword)){
                            array_push( $gptkeyword, array(
                                "role" => "assistant",
                                "content" => $mess->choices[0]->message->content
                            ));
                            setcookie('last_five_prompt', (json_encode($gptkeyword)) , time() + (60000), "/");
                        }
                    }
