 ...
 * @return array
 */
function corporate_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'corporate_post_classes', 10, 3 );
 ...
 * @return void
 */
function corporate_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'corporate_pingback_header' );
 ...
 * @return void
 */
function corporate_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'corporate_supports_js' );
 ...
 * @return array
 */
function corporate_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'corporate_comment_form_defaults' );
 ...
 * @return bool
 */
function corporate_can_show_post_thumbnail() {
	/**
	...
	return apply_filters(
		'corporate_can_show_post_thumbnail',
 ...
 * @return int
 */
function corporate_get_avatar_size() {
	return 60;
}
 ...
 * @since corporate 1.0
 */
function corporate_continue_reading_text() {
 ...
 * @since corporate 1.0
 */
function corporate_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . corporate_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'corporate_continue_reading_link_excerpt' );
 ...
 * @since corporate 1.0
 */
function corporate_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . corporate_continue_reading_text() . '</a></div>';
	}
}

// Filter the excerpt more link.
add_filter( 'the_content_more_link', 'corporate_continue_reading_link' );

if ( ! function_exists( 'corporate_post_title' ) ) {
	/**
 ...
 * @return string
 */
function corporate_get_icon_svg( $group, $icon, $size = 24 ) {
	return corporate_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function corporate_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? corporate_get_icon_svg( 'ui', 'arrow_right' ) : corporate_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? corporate_get_icon_svg( 'ui', 'arrow_left' ) : corporate_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'corporate_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function corporate_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'corporate_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'corporate_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'corporate_generate_css' ) ) {
	...
	return corporate_generate_css( // @phpstan-ignore-line.
 ...
 */
function corporate_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 *
 * @since corporate 1.0
 * @since corporate 1.4 Corrected parameter name for `$output`,
 ...
 */
function corporate_password_form( $output, $post = 0 ) {
	$post   = get_post( $post );
...
add_filter( 'the_password_form', 'corporate_password_form', 10, 2 );
 ...
 */
function corporate_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'corporate_get_attachment_image_attributes', 10, 3 );
